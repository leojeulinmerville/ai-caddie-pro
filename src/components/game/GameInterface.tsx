...
export function GameInterface({
  ...
  return <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Button onClick={onAddStroke} className="bg-primary hover:bg-primary/90 text-primary-foreground py-6 text-lg">
          +1
        </Button>
        <Button onClick={onUndoStroke} variant="outline" className="border-hs-sand text-hs-ink hover:bg-hs-sand/10 py-6 text-lg" disabled={currentHoleStrokes.length === 0}>
          <Undo className="w-5 h-5 mr-2" />
          Undo
        </Button>
      </div>

      <Button onClick={onFinishHole} variant="secondary" disabled={currentHoleStrokes.length === 0} className="w-full bg-hs-green-200 hover:bg-hs-green-900 py-6 text-lg text-green-950 bg-amber-300 hover:bg-amber-200">
        ...
        <CardContent className="pt-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              {isRecording && <p className="text-xs text-hs-sand mt-1">
                  Dites "play", "finish" ou "undo"...
                </p>}
            </div>
            <Button onClick={onToggleVoiceRecording} variant={isRecording ? "destructive" : "outline"} size="sm" className={`${isRecording ? 'animate-pulse bg-red-500 hover:bg-red-600' : 'border-hs-green-100 text-hs-green-100 hover:bg-hs-green-100 hover:text-white'}`}>
              {isRecording ? <>
